...
from engine_lifecycle import rebuild_wallet
...
def startup_fills(user: str) -> list[Fill]:
    ...
    if not isinstance(data, list):
        raise RuntimeError(
            f"Unexpected userFills response for {user}"
        )

    fills = [
        Fill.from_raw(user, raw)
        for raw in data
    ]
...
def load_startup_state() -> None:
    ...
    for wallet, state in wallet_states.items():
        (
            ...
            state.snapshot_capital,
        ) = snapshot_portfolio(wallet)
    ...
    if FOLLOWER:
        (
            follower_state.positions,
            _,
        ) = snapshot_portfolio(FOLLOWER)

    log.info(
        "[STARTUP] leaders=%s assets=%s follower_positions=%s execution_enabled=%s",
        len(LEADERS),
        sum(
            len(state.lifecycles)
            for state in wallet_states.values()
        ),
