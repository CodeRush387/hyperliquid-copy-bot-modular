...
async def wait_for_follower_position(coin: str, predicate) -> bool:
    ...
    try:
        await asyncio.wait_for(
            wait_loop(),
            timeout=EXECUTION_CONFIRM_TIMEOUT,
        )
        return True
...
def close_order(coin: str, position: Decimal) -> Any:
    ...
    price = runtime.exchange._slippage_price(
        coin,
        is_buy,
        SLIPPAGE,
        None,
...
def open_order(lifecycle: Lifecycle) -> Any:
    ...
    if price is None or price <= ZERO:
        raise RuntimeError(
            f"No gRPC BBO midpoint available for {lifecycle.coin}"
        )

    precision = runtime.size_decimals.get(lifecycle.coin, 6)
    size = (
        math.floor(
            float(FIXED_NOTIONAL_USD / price) * 10**precision
        )
        / 10**precision
    )
    ...
    if size <= 0:
        raise RuntimeError(
            f"Calculated zero order size for {lifecycle.coin}"
        )
...
async def execute_initial_entry(
    state: WalletState,
    ...
    async with runtime.trade_lock:
        ...
        if not order_ok(result):
            log.error(
                "[ROTATION_ABORTED_CLOSE_FAILED] initial open rejected result=%s",
                result,
            )
            return Decision.CLOSE_FAILED

        expected_sign = (
            1 if challenger.direction == "LONG" else -1
        )

        confirmed = await wait_for_follower_position(
            challenger.coin,
            lambda value: sign(value) == expected_sign,
        )

        if not confirmed:
            log.error(
                "[ROTATION_ABORTED_CLOSE_FAILED] initial open not confirmed"
            )
            return Decision.CLOSE_FAILED
...
async def execute_rotation(
    ...
    async with runtime.trade_lock:
        ...
        held_position = runtime.follower_state.positions.get(
            held_coin,
            ZERO,
        )
        ...
        if held_position != ZERO:
            ...
            if not await wait_for_follower_position(
                held_coin,
        ...
        if not order_ok(open_result):
            log.error(
                "[ROTATION_OPEN_FAILED] wallet=%s result=%s",
                state.wallet,
                open_result,
            )
            return Decision.CLOSE_FAILED
        ...
        if not confirmed:
            log.error(
                "[ROTATION_OPEN_FAILED] wallet=%s reason=entry_not_confirmed",
                state.wallet,
            )
            return Decision.CLOSE_FAILED
